// Espejo de api/src/lib/addon-services.ts — este repo no es un monorepo con paths compartidos
// entre api/ y este Angular, así que no hay forma de importar ese archivo directo. Si un precio o
// paquete cambia, hay que replicarlo a mano en los dos lados (mismo comentario en ambos).
//
// Precios en dólares (USD), igual que los planes de suscripción — unificado en la revisión del
// tarifario comercial. El nombre de campo `unitPriceDOP` queda igual por retrocompatibilidad con
// el espejo de api/src/lib/addon-services.ts, pero desde este cambio son montos en USD.
...
export const ADDON_PACKAGES_LIST: AddOnPackageDefinition[] = Object.values(ADDON_PACKAGES);
